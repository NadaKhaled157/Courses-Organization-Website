...
<div style="height: 200px; background-color: #1c1f4c;">
    <div class="d-lg-flex justify-content-end d-sm-none">
        <div class="card ms-auto my-3 col-sm-6 position-fixed" style="width: 20rem; margin-right: 100px;">
            <?php echo "<img src='Photos/Courses/" .$course['img']."' class='card-img-top' alt='".$course['img']."'>";?>
            <div class="card-body">
...
<!--                <p class="card-text">Course Overview:</p>-->
                ...
                <?php
                if (isset($_SESSION['email']))
                echo "<a class='btn btn-vintage d-block' data-bs-toggle='modal' data-bs-target='#staticBackdrop'>Enroll</a>
                <a href='enroll-save.php?type=save&user=". $_SESSION['email']."&course=".$course['id']."' type='button' class='btn btn-outline-sec d-block mt-2'>Save For Later</a>";
                else echo "<span class='d-block' tabindex='0' data-bs-toggle='popover' data-bs-trigger='hover focus' data-bs-content='Please login to save a course or enroll in it.'>
<a class='btn btn-vintage d-block disabled' data-bs-toggle='modal' data-bs-target='#staticBackdrop'>Enroll</a>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to enroll in <strong><?php echo $course['title']?></strong> Course?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <?php echo "<a href='enroll-save.php?type=enroll&user=". $_SESSION['email']."&course=".$course['id']."' type='button' class='btn btn-vintage'>Enroll Now</a>";?>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal fade" id="reviewsWindow" tabindex="-1" aria-labelledby="reviewsWindowLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <?php
                        $query= "SELECT * FROM reviews WHERE CourseID='".$course['id']."' ";
                        ...
                        if($result->num_rows>0){
                            while($row=$result->fetch_assoc()){
                                ...
                                echo "<h6 style='color:#1c1f4c;'>$username <i style='color:#00848c;'>".$row['rating']."/5 ✩</i></h6>";
                                echo $row['review'];
                                echo "<hr>";
                            }
...
<?php
$query="SELECT * from syllabus WHERE CourseID= '".$course['id']."' ";
$result3=$conn->query($query);
if($result3->num_rows>0)
$syllabus=$result3->fetch_assoc();
else {
    $syllabus['col1']= "Content One";
    $syllabus['col2']= "Content Two";
    $syllabus['col3']= "Content Three";
}
?>
...
<div class="card ms-sm-3 ms-md-5 mt-4" style="width:20rem; background-color: #faf1d7" id="card1">
    <div class="card-body">
        ...
        <ul class="list-group list-group-flush" style="background-color: #faf1d7">
            <li class="list-group-item" style="background-color: #faf1d7; color:#1c1f4c;"><?php echo $syllabus['col1']?></li>
            <li class="list-group-item" style="background-color: #faf1d7; color:#1c1f4c;"><?php echo $syllabus['col2']?></li>
            <li class="list-group-item" style="background-color: #faf1d7; color:#1c1f4c;"><?php echo $syllabus['col3']?></li>
        </ul>
    </div>
</div>
